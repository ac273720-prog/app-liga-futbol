(() => {
  ...
  function renderClubPanel(){
    ...
    const team=S.teams.find(t=>t.id===S.u.team_id);if(!team){box.innerHTML='<div class="empty">No se encontró tu club.</div>';return}
  ...
  async function uploadClubLogo(e){
    e.preventDefault();const msg=document.querySelector('#clubLogoMsg'),file=document.querySelector('#clubLogoFile')?.files?.[0];
    try{
      if(S.u?.role!=='team_admin'||!S.u.team_id)throw Error('Solo un administrador de club puede cambiar este logo.');if(!file)throw Error('Selecciona una imagen.');if(file.size>5*1024*1024)throw Error('El logo no puede superar 5 MB.');if(!['image/jpeg','image/png','image/webp'].includes(file.type))throw Error('Formato no permitido. Usa JPG, PNG o WebP.');
      msg.textContent='Subiendo logo...';msg.className='msg';const ext=(file.name.split('.').pop()||'jpg').toLowerCase().replace(/[^a-z0-9]/g,'');const path=`${S.u.team_id}/logo-${Date.now()}.${ext}`;
      const {error:upErr}=await sb.storage.from('team-logos').upload(path,file,{cacheControl:'3600'});if(upErr)throw upErr;const {data:urlData}=sb.storage.from('team-logos').getPublicUrl(path);const url=urlData.publicUrl;
  ...
  function installAdminGate(){
    const adminBtn=document.querySelector('#adminLoginBtn'),ownerBtn=document.querySelector('#ownerLoginBtn'),loginForm=document.querySelector('#loginForm'),backBtn=document.querySelector('#backPublic');
    if(adminBtn)adminBtn.onclick=()=>document.querySelector('#loginModal')?.classList.remove('hidden');if(ownerBtn)ownerBtn.onclick=()=>document.querySelector('#loginModal')?.classList.remove('hidden');if(loginForm)loginForm.addEventListener('submit',()=>sessionStorage.setItem('adminGate','1'),true);
    if(backBtn)backBtn.onclick=async()=>{const aid=S.a;if(aid)localStorage.setItem('publicAssociation',aid);sessionStorage.removeItem('adminGate');await sb.auth.signOut();S.u=null;await startPublic()};
    setTimeout(async()=>{const {data:{session}}=await sb.auth.getSession();const gate=sessionStorage.getItem('adminGate')==='1';if(session&&!gate){await sb.auth.signOut();S.u=null;await startPublic()}},500);
  }

  function installManualVenue(){
    const form=document.querySelector('#fixtureForm'),old=document.querySelector('#fVenue');if(!form||!old||old.dataset.manualVenue==='1')return;const label=old.closest('label');if(!label)return;
    ...
    form.onsubmit=async e=>{e.preventDefault();if(!canSchedule())return flash('#fixtureMsg','No tienes permiso');if(!document.querySelector('#fCompetition').value)return flash('#fixtureMsg','Primero debes tener una liga creada para esta asociación');if(document.querySelector('#fHome').value===document.querySelector('#fAway').value)return flash('#fixtureMsg','Local y visita deben ser distintos');
  ...
  function generalStatus(pos,total,competitionName){
    const champion=pos===1?'🏅 Campeón general':'';
    if(!isAfacon())return champion||'—';
    const c=norm(competitionName);let extra='';
    if(c.includes('liga a')){
  ...
  function decoratePublicGeneral(){
    const body=document.querySelector('#pubGeneral');if(!body)return;const table=body.closest('table'),head=table?.querySelector('thead tr');if(!head)return;
    if(!head.querySelector('[data-qualification]')){const th=document.createElement('th');th.dataset.qualification='1';th.textContent='Situación';head.appendChild(th)}
  ...
  function ensureAdminGeneral(){
    const panel=document.querySelector('#p-tables');if(!panel)return;let card=document.querySelector('#adminGeneralCard');if(card)return;
  ...
  function installQualificationViews(){
    decorateSeriesTable(document.querySelector('#pubStandings'));decorateSeriesTable(document.querySelector('#standings'));decoratePublicGeneral();ensureAdminGeneral();scheduleAdminGeneral();
    const pubSeriesBody=document.querySelector('#pubStandings');if(pubSeriesBody&&!pubSeriesBody.dataset.qualWatch){pubSeriesBody.dataset.qualWatch='1';new MutationObserver(()=>decorateSeriesTable(pubSeriesBody)).observe(pubSeriesBody,{childList:true})}
    const adminSeriesBody=document.querySelector('#standings');if(adminSeriesBody&&!adminSeriesBody.dataset.qualWatch){adminSeriesBody.dataset.qualWatch='1';new MutationObserver(()=>{decorateSeriesTable(adminSeriesBody);scheduleAdminGeneral()}).observe(adminSeriesBody,{childList:true})}
    const pubGen=document.querySelector('#pubGeneral');if(pubGen&&!pubGen.dataset.qualWatch){pubGen.dataset.qualWatch='1';new MutationObserver(()=>decoratePublicGeneral()).observe(pubGen,{childList:true})}
  ...
  function init(){
    if(!waitForApp())return setTimeout(init,100);ensurePanels();installAdminGate();installManualVenue();installQualificationViews();const nav=document.querySelector('#nav');new MutationObserver(()=>{ensurePanels();patchNav();installManualVenue();installQualificationViews()}).observe(nav,{childList:true});
    setInterval(()=>{decorateSeriesTable(document.querySelector('#pubStandings'));decoratePublicGeneral();if(!document.querySelector('#adminView').classList.contains('hidden')){patchNav();installManualVenue();installQualificationViews()}},900);
  }
  init();
})();