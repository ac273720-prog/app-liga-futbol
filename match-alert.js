...
function ensureInstallButton(){let buttons=findInstallButtons();if(buttons.length>1){buttons.slice(1).forEach(el=>el.remove());buttons=buttons.slice(0,1)}let btn=buttons[0]||null;if(!btn){const host=document.querySelector('#publicView .top-actions');if(!host)return;btn=document.createElement('button');btn.id='installAppBtn';btn.type='button';btn.textContent='📲 Instalar app';host.appendChild(btn)}btn.dataset.pwaReady='1';btn.style.cssText='display:inline-flex!important;align-items:center;justify-content:center;gap:7px;background:linear-gradient(135deg,#ffb300,#ff7a00)!important;color:#1d1600!important;border:2px solid #ffe082!important;border-radius:12px!important;padding:10px 16px!important;font-weight:950!important;font-size:14px!important;box-shadow:0 4px 12px rgba(255,122,0,.35)!important;cursor:pointer!important;';btn.onclick=async e=>{e.preventDefault();trackInstallEvent('install_click');if(isStandalone()){showInstallHelp('Linares Score ya está instalada en este dispositivo.');return}if(installPrompt){const p=installPrompt;installPrompt=null;await p.prompt();try{const choice=await p.userChoice;if(choice?.outcome==='accepted')trackInstallEvent('install_accepted')}catch(_){}return}if(isInstagram()){if(isIos()){showInstallHelp('Instagram no permite instalar aplicaciones dentro de su navegador. Toca los tres puntos de arriba, elige “Abrir en Safari”; después toca Compartir y “Añadir a pantalla de inicio”.')}else{const cleanUrl=`${location.host}${location.pathname}${location.search}${location.hash}`;const openChrome=()=>{location.href=`intent://${cleanUrl}#Intent;scheme=https;package=com.android.chrome;end`};showInstallHelp('Instagram no permite instalar aplicaciones dentro de su navegador. Abre Linares Score en Chrome y vuelve a tocar “Instalar app”.',openChrome)}return}if(isIos())showInstallHelp('En iPhone o iPad, toca Compartir y luego “Añadir a pantalla de inicio”.');else showInstallHelp('Abre el menú de tu navegador y elige “Instalar aplicación” o “Añadir a pantalla de inicio”.')}}
